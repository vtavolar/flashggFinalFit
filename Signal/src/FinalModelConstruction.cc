  ...
  procs_(procList)
{
  ...
  processNamesFromIndexes.insert(std::pair<int,std::string >(11, "ggh"));
  processNamesFromIndexes.insert(std::pair<int,std::string >(12, "vbf"));
  //  processNamesFromIndexes.insert(std::pair<int,std::string >(13, "vh"));
  processNamesFromIndexes.insert(std::pair<int,std::string >(14, "tth"));
  ...
  brSpline = graphToSpline(Form("fbr_%dTeV",sqrts_),brGraph,MHref);
  
  // string procs[8] = {"ggh","vbf","wzh","wh","zh","tth","gg_grav","qq_grav"} ;//Don't want this hard-coded.
  for (unsigned int i=0; i<procs_.size(); i++){
    std::cout<<"getsigmagraph for proc "<<procs_[i]<<std::endl;
    TGraph *xsGraph = norm->GetSigmaGraph(procs_[i].c_str());
    RooSpline1D *xsSpline = graphToSpline(Form("fxs_%s_%dTeV",procs_[i].c_str(),sqrts_),xsGraph,MHref);
...
std::string FinalModelConstruction::getProcessNameFromIndex(int index){
  if( (processNamesFromIndexes.find(index) == processNamesFromIndexes.end() ) ){
    std::cout<<"ERROR: could not find process name for this process index!"<<std::endl;
    exit(1);
  }
  else{
    return processNamesFromIndexes[index];
  }
}

void FinalModelConstruction::getNormalization(std::string cut ){
  string catname;
  //remove leading and trailing spaces from cut
  cut = cut.substr( cut.find_first_not_of(" \t"), (cut.find_last_not_of(" \t") - cut.find_first_not_of(" \t") + 1));
  ...
  int theindex = 0;
  if(cut != "1"){
    theindex = std::stoi( cut.substr(cut.length() - 2 ) ); //processIndex has to be 2 digits
    procLowerCase_ = getProcessNameFromIndex(theindex);
    if(cut.find("!=") != std::string::npos  && procLowerCase_ == "ggh" ){
      procLowerCase_ = "hx";
    }
    std::cout<<"proclowercase in getnormalization "<<procLowerCase_<<std::endl;
  }
  ...
  std::cout<<"retrieving xsspline for proc "<<proc_<<std::endl;
  xs->Print();
  ...
  for (int m =120; m<131; m=m+5){
    MH->setVal(m); 
    if(verbosity_ >1) std::cout << "[INFO] MH " << m <<  " -- br " << (brSpline->getVal()) << " - ea "  <<  (eaSpline->getVal()) << " intL= " << intLumi->getVal() << " xs " << xs->getVal() << "norm " << (brSpline->getVal())*(eaSpline->getVal())*(xs->getVal()) << "predicted events " <<  (brSpline->getVal())*(eaSpline->getVal())*(xs->getVal())*intLumi->getVal() << " -- finalNorm " << finalNorm->getVal() << std::endl;
  }
