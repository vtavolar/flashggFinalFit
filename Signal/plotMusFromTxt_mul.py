...
ROOT.myColorG1   = ROOT.TColor.GetColor("#646059")
...
#####                print graphLine.GetX()
######                graphLine.SetPoint(graphLine.GetN(), getListFromBuffer(graphLine.GetX(),graphLine.GetN())[-1], 0.0)
######                graphLine.SetPointError(graphLine.GetN(), getListFromBuffer(graphLine.GetEXlow(), graphLine.GetN())[-1], getListFromBuffer(graphLine.GetEXhigh(),graphLine.GetN())[-1], 0.0, 0.0)
#####                graphLine.RemovePoint(graphLine.GetN()-1)
#####                if obs == "AbsDeltaPhiGgJjEta4p7":
#####                    box = TBox(3.05,gPad.GetUymin(),3.15,200.0 )
#####                else:
#####                    box = TBox(3.05,gPad.GetUymin(),3.15,70.0 )
#####                shadedGray= TColor.GetColorTransparent(myColorG1, 0.30/4.0)
#####                box.SetFillColor(shadedGray)
#####                box.Draw()
#####                otherobjs[pr]=box
                
###                    sortedData[-1]['y%s'%pr] = 0.
###                    sortedData[-1]['y%sUc'%pr] = 0.
...
#        ratio.GetXaxis().SetLabelSize(0.107)
        ...
        box=TBox()
#####        if obs == "AbsDeltaPhiGgJjEta4p7VBFlike" or obs == "AbsDeltaPhiGgJjEta4p7":
#####            ratio.RemovePoint(ratio.GetN()-1)
#####            box = TBox(3.05,gPad.GetUymin(),3.15,gPad.GetUymax())
#####            shadedGray= TColor.GetColorTransparent(myColorG1, 0.3)
#####            box.SetFillColor(shadedGray)
        ratio.Draw("ap")
        box.Draw()
        pads[1].Update()
