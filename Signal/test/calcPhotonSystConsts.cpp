...
Double_t effSigma(TH1 * hist)
{
  hist->Print();
	...
	if(verbosity_ || 1)	std::cout<< "[INFO] " << (hist->GetName()) << " has effSigma " << widmin << std::endl;
		...
		if (isFlashgg_){
			...
			if ((up) && (down) && (nominal)) {
			  ...
			  //				cout << "[ERROR] - at least one of histograms " << Form("tagsDumper/histograms/%s_%sDown01sigmamass",name.c_str(),syst.c_str()) << " "<<Form("tagsDumper/histograms/%s_%sUp01sigmamass",name.c_str(),syst.c_str()) <<", " <<Form("tagsDumper/histograms/%smass",name.c_str()) << std::endl;
				cout << "[ERROR] - at least one of histograms " << Form("histograms/%s_%sDown01sigmamass",name.c_str(),syst.c_str()) << " "<<Form("histograms/%s_%sUp01sigmamass",name.c_str(),syst.c_str()) <<", " <<Form("histograms/%smass",name.c_str()) << std::endl;
...
int main(int argc, char *argv[]){
  ...
  inWS_ = new WSTFileWrapper(infilenamesStr_,"cms_hgg_13TeV");
